...
### Fixed
- Script self-update now creates temporary files using a macOS-compatible `mktemp` template, so accepting an update no longer aborts with `mktemp failed`.
- Script restarts preserve any command-line arguments after a successful self-update.
